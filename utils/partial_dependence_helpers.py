...
API_KEY = "NjQzMTZmNDY0ZjU1MDVkNjcxZGE3NzdjOmhFY2RyRlhOTlFjQUNnc2h4elhwUHhqSmI2b3VTR2pydUl4Uk5lNFVnaEk9"
...
def extract_prediction_values(df: pd.DataFrame, project) -> pd.DataFrame:
    return (
        df.assign(value=lambda x: x.predictionValues.apply(get_predicted_label)
        )
...
#             preds = get_realtime_predictions(df, deployment_id)
#             self.col = list(get_column_name_mappings(self.project.id))[1]
        else:
    ...
    def get_feature_impact(self, n=None):
        ...
        try:
            ...
            try:
                ...
                shap_impact_job = dr.ShapImpact.create(project_id=self.project.id, model_id=self.model.id)
    ...
    def create_pdp_plot(
        ...
        if feature_2:
            ...
            if self.weights:
                ...
                two_way_pdp = (
                    two_way_pdp
                    .groupby([feature_1, feature_2])[col]
                    .agg(['mean','std','count'])
                    .reset_index()
                    .sort_values(by=feature_1, ascending=True)
                )
        ...
        if include_ice & (feature_2 is not None):
            ...
            for idx, f in enumerate(two_way_pdp[feature_2].unique()):
                if isinstance(f, pd._libs.interval.Interval):
                    preds_temp = preds.loc[(preds[feature_2]>=f.left) & (preds[feature_2]<f.right),:]
                else:
