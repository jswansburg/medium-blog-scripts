...
def prep_pe_data(
    ...
    #         "actual_value": get_missing_counts,
    #     })
    #     .reset_index()
    # )
    ...
    resampled = (
        ...
        .agg(
            ...
            missing_values = ("actual_value", get_missing_counts),
        )
        .reset_index()
    )
    ...
    normalized["missing_pct"] = (
        normalized["missing_values"] / normalized["row_count"]
    )

    normalized['normalized_diff'] = normalized.groupby(['feature_name'])['strength_normalized'].diff()
...
def plot_pe_over_time(
    ...
    """
    tickformat = ",.5" if dr.Project.get(project_id).target_type=="Regression" else ",.0%"
    ...
    y =  "normalized_diff" if plot_differences else "strength_normalized"
    ...
    for i, trace in enumerate(bar["feature_name"].unique()):
        ...
        median_val = (
            ...
            else "<br>Median Value</b>: %{customdata[0]: .2f}"
        )
        fig.add_traces(
            go.Bar(
                x=dft[date_col],
                ...
                customdata=dft[
                    ["actual_value", "strength", "feature_name", "missing_pct"]
                ],
                hovertemplate="<br>Period</b>: %{x}"
                + "<br>Feature</b>: %{customdata[2]}"
                + "<br>Strength</b>: %{y: .2}"
                + median_val
                + "<br>Percent Missing</b>: %{customdata[3]: .1%}"
                + "<extra></extra>",
                hoverlabel={
                    "align": "left",
                    "bgcolor": "white",
        ...
        for i, trace in enumerate(df[feature].unique()):
            ...
            fig.add_traces(
                go.Bar(
                    ...
                    name=trace,
                    marker = dict(
                        line=dict(color='rgba(0,0,0)', width=0.75)
                    ),
                    #marker_color=marker_color[trace],
                    opacity=0.5,
                )
